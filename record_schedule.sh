...
Usage:
    ...
    -s|--silent                  Run silently unless we encounter an error
...
function backup_old_data(){
	OBJECT_FILE="/home/nguyennd/Workspaces/bash-script-template/object_lifecycle.csv"
	EVENT_FILE="/home/nguyennd/Workspaces/bash-script-template/object_event.csv"
	...
	BACKUP_FILE="$(date --date="$ST day ago" '+%Y%m%d%H%M%S')_$(date --date="$ET day ago" '+%Y%m%d%H%M%S').tar.gz"
...
function erase_old_data(){
	...
	psql $RECORD_DB_NAME $RECORD_DB_USER -p $RECORD_DB_PORT  -h $RECORD_DB_HOST -c "delete from $RECORD_LIFE_CIRCLE where lifetime && $TIME_RANGE";
}
